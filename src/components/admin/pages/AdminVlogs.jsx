...
const AdminVlogs = () => {
  ...
  const handleDelete = (id) => {
    axios.delete(`/admin/vlogs/${id}`).then((res) => {
      setVlogs(vlogs.filter((vlog) => vlog._id !== id));
    });
  };
  return (
    <div id="main" className="bg-gray-100 w-full h-screen flex flex-col sm:flex-row">
      ...
      <div className="container h-full w-full sm:w-[80%] mx-auto py-4 sm:py-8 px-4 overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl sm:text-3xl font-bold text-black">Vlogs</h1>
          <Link
            to="/admin/vlogs/add"
        ...
        {vlogs.length === 0 ? (
          <div className="flex justify-center items-center mt-10">
            <h1 className="text-2xl font-bold">No vlogs found, add one</h1>
          </div>
        ) : (
          <>
